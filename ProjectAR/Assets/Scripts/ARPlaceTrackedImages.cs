...
public class ARPlaceTrackedImages : MonoBehaviour
{
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs)
    {
        ...
        foreach (var trackedImage in eventArgs.added)
        {
            ...
            foreach (var curPrefab in ArPrefabs)
            {
                    ...
                    && !_instantiatedPrefabs.ContainsKey(imageName))
                {
                    ...
                    newPrefab.transform.GetChild(0).GetComponent<rotation>().enabled = false;
                    newPrefab.transform.GetChild(0).GetComponent<RotateToCamera>().enabled = true;
        ...
        foreach (var trackedImage in eventArgs.updated)
        {
            ...
            // Store plant in inventory if not yet found
            var imageName = trackedImage.referenceImage.name;
            if (!File.Exists(fileName)) File.Create(fileName).Close();

            string[] plantsRecorded = File.ReadAllLines(fileName);  
            if (!Array.Exists(plantsRecorded, e => e == imageName))
            {
                File.AppendAllText(fileName, imageName + Environment.NewLine);
            }
        }
